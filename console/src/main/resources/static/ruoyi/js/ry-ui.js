...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            submit2: function(url, type, dataType, data, callback) {
                var config = {
                    url: url,
                    type: type,
                    dataType: dataType,
            ...
            post2: function(url, data, callback) {
                $.operate.submit2(url, "post", "json", data, callback);
            },
            // get请求传输
            ...
            remove: function(id) {
            	...
            	$.modal.confirm("确定删除该条" + table.options.modalName + "信息吗？", function() {
                    var url ;
                    if (table.options.removeUrl.indexOf("${id}")==-1){
                        url = $.common.isEmpty(id) ? table.options.removeUrl : table.options.removeUrl.replace("{id}", id);
                    }else{
                        url = table.options.removeUrl+"?id="+id;
                    }
            ...
            removeMethod: function(id,method) {
                table.set();
                $.modal.confirm("确定删除该条" + table.options.modalName + "信息吗？", function() {
                    var url ;
                    if (table.options.removeUrl.indexOf("${id}")==-1){
            ...
            openAdd: function(width,height){
                table.set();
                $.modal.open("添加" + table.options.modalName, table.options.createUrl,width,height);
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
            	table.set();
            ...
            editFull: function(id) {
            	...
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
                    if (table.options.updateUrl.indexOf("${id}")==-1){
                        url = table.options.updateUrl.replace("{id}", id);
                    }else{
                        url = table.options.updateUrl+"?id="+id;
                    }
            	} else {
            	    if(table.options.type == table_type.bootstrapTreeTable) {
            	        var row = $("#" + table.options.id).bootstrapTreeTable('getSelections')[0];
            ...
            // 保存信息 刷新表格
            save: function(url, data, callback,isStream) {
            	var config = {
        	        url: url,
        	        type: "post",
        	        dataType: "json",
            ...
            // 保存选项卡信息
            saveTab: function(url, data, callback,isStream) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
...
function openView (title, url, width, height, callback) {
    // 如果是移动端，就使用自适应大小弹窗
    if ($.common.isMobile()) {
        width = 'auto';
        height = 'auto';
