...
import com.github.fashionbrot.core.dao.MenuDao;
...
public class MenuService {
    ...
    public boolean checkPermissionUrl(Object handler, HttpServletRequest request,LoginModel model) {
    ...
    public List<Menu> loadAllMenu2(Long roleId,boolean isSuper) {
        List<Menu> checkedList = null;
        if (isSuper){
            QueryWrapper q = new QueryWrapper();
            q.in("menu_level",Arrays.asList(1,2));
            checkedList = menuDao.queryAll(q);
        }else{
            checkedList = menuDao.selectMenuRole(roleId);
        }
